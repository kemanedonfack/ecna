...
@endif
    <img class="mr-3" width="70px" src="{{asset('storage') . '/' . $comment->commenter->avatar ?? $comment->avatar }}" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar">
    <div class="media-body">
        <h5 class="mt-3 mb-1 font-weight-bold">{{ $comment->commenter->name ?? $comment->guest_name }} </h5>
        ...
        <div class="border-bottom border-gray pb-3">
            @can('reply-to-comment', $comment)
                <button data-toggle="modal" data-target="#reply-modal-{{ $comment->id }}" class="btn btn-sm btn-link text-uppercase">
                    <i class="fa fa-reply fa-1x mr-1"></i> Repondre
                </button>
            @endcan
                    ...
                    <div class="modal-content">
                        <form method="POST" action="{{ route('comments.update', $comment->id) }}">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-sm btn-outline-secondary text-uppercase" data-dismiss="modal">Annuler</button>
                                <button type="submit" class="btn btn-sm btn-outline-success text-uppercase">Modifier</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form method="POST" action="{{ route('comments.reply', $comment->id) }}">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-sm btn-outline-secondary text-uppercase" data-dismiss="modal">Annuler</button>
                                <button type="submit" class="btn btn-sm btn-outline-success text-uppercase">Repondre</button>
                            </div>
                        </form>
                    </div>
                </div>
